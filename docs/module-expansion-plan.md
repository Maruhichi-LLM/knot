...
7. [監査モジュール](#7-監査モジュール)
8. [Knot Event 拡張: イベント収支管理機能](#8-knot-event-拡張-イベント収支管理機能)
...
## 8. Knot Event 拡張: イベント収支管理機能

### 概要
**Knot Eventのオプション機能**として、イベントごとの収入・支出を個別に管理し、最終的に本会計に取り込む。
Knot Storeでon/off切り替えが可能な拡張機能として提供。

### モジュール設計思想
- **独立モジュールではなく、Knot Eventの拡張機能**
  - イベント管理と収支管理は密接に関連するため、一体化した方が使いやすい
  - 小規模団体は基本のイベント管理のみ、中〜大規模団体は収支管理を有効化
  - Knot Storeで簡単に機能を追加できる設計
...
#### 1. イベント作成時の収支管理設定
- イベント作成時に「収支管理を使う」チェックボックス
- 有効化するとEventBudgetが自動作成
- 予定収入・予定支出の入力
...
├─ 「このEventについて話す」ボタン
└─ 【event-budget有効時のみ】
    └─ 収支サマリー（収入 ¥XX,XXX / 支出 ¥XX,XXX / 損益 ¥XX,XXX）
```

#### イベント詳細ページ (/events/[id]) ※新規作成
```
タブ構成:
├─ 基本情報タブ
│   ├─ イベント詳細
...
### ワークフロー
1. Knot Storeで「Event Budget」機能を有効化
2. イベント作成時に「収支管理を使う」をチェック
3. イベント期間中に収入・支出を記録
4. イベント終了後、収支を確定
5. 承認者が収支を確認・承認
6. 「本会計に取り込む」ボタンで一括転記
7. 取り込み完了後、イベント収支は読み取り専用に
8. Knot Accountingで全体の会計管理
...
### Knot Eventとの統合メリット
1. **一体的なUX**: イベント管理と収支管理がシームレス
2. **段階的導入**: 小規模団体は基本機能のみ、成長に合わせて拡張
3. **データの一貫性**: Event→EventBudget→Ledgerと明確な流れ
4. **Knot Chatとの連携**: イベントスレッドで収支の議論が可能
5. **既存設計との整合性**: ToDo/Accounting/Documentと同じ連携思想
...
### モジュール設計の特徴
- **基本モジュール**: Chat、ToDo、Event、Accounting、Document
- **拡張機能**: Event Budget（EventのオプションとしてKnot Storeで有効化）
- **追加モジュール**: 議事録、申請フロー、プロジェクト管理、通知、ダッシュボード、FAQ、監査

各モジュールは独立して機能しつつ、Knot Chatを中心に有機的に連携し、団体運営の全体最適を実現します。
特にEvent Budgetのような拡張機能は、Knot Storeを通じて段階的に導入できる設計とすることで、団体の成長に合わせた柔軟な運用が可能になります。
